...
public final class AdvancedSettingsFragment extends SubScreenFragment {
    ...
    private void setupKeyRepeatStartTimeoutSettings() {
        ...
        if (pref == null) {
            Log.w(TAG, "Preference not found: " + Settings.PREF_KEY_REPEAT_START_TIMEOUT);
            return;
        }
    ...
    private void setupKeyRepeatIntervalSettings() {
        ...
        if (pref == null) {
            Log.w(TAG, "Preference not found: " + Settings.PREF_KEY_REPEAT_INTERVAL);
            return;
        }
        final SharedPreferences prefs = getSharedPreferences();
        final Resources res = getResources();
    ...
    private void setupVibrationDurationSettings() {
        ...
        if (pref == null) {
            Log.w(TAG, "Preference not found: " + Settings.PREF_VIBRATION_DURATION);
            return;
        }
        final SharedPreferences prefs = getSharedPreferences();
        final Resources res = getResources();
    ...
    private void setupKeyPreviewLingerTimeoutSettings() {
        ...
        if (pref == null) {
            Log.w(TAG, "Preference not found: " + Settings.PREF_KEY_PREVIEW_LINGER_TIMEOUT);
            return;
        }
        final SharedPreferences prefs = getSharedPreferences();
        final Resources res = getResources();
    ...
    private void setupBottomOffsetLandscapeSettings() {
        ...
        if (pref == null) {
            Log.w(TAG, "Preference not found: " + Settings.PREF_BOTTOM_OFFSET_LANDSCAPE);
            return;
        }
        final SharedPreferences prefs = getSharedPreferences();
        final Resources res = getResources();
    ...
    private void setupClipboardMaxClipsSettings() {
        ...
        if (pref == null) {
            Log.w(TAG, "Preference not found: " + Settings.PREF_CLIPBOARD_MAX_CLIPS);
            return;
        }
        final SharedPreferences prefs = getSharedPreferences();
        final Resources res = getResources();
